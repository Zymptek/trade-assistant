import { getChats } from '@/lib/actions/chat'
import { getCurrentUser } from '@/lib/auth/session'
import { Chat } from '@/lib/types'
import { cache } from 'react'
import { ClearHistory } from './clear-history'
import HistoryItem from './history-item'

type HistoryListProps = {
  userId?: string
}

const loadChats = cache(async (userId?: string) => {
  return await getChats(userId)
})

export async function HistoryList({ userId }: HistoryListProps) {
  // Get current user from session, or use provided userId, or fallback to anonymous
  const user = await getCurrentUser()
  const effectiveUserId = userId || user?.id || 'anonymous'
  
  const chats = await loadChats(effectiveUserId)

  return (
    <div className="flex flex-col flex-1 space-y-3 h-full">
      <div className="flex flex-col space-y-0.5 flex-1 overflow-y-auto">
        {!chats?.length ? (
          <div className="text-foreground/30 text-sm text-center py-4">
            No search history
          </div>
        ) : (
          chats?.map(
            (chat: Chat) => chat && <HistoryItem key={chat.id} chat={chat} />
          )
        )}
      </div>
      <div className="mt-auto">
        <ClearHistory empty={!chats?.length} userId={effectiveUserId} />
      </div>
    </div>
  )
}